export const SIGN_UP_REQUEST = "SIGN_UP_REQUEST"
export const SIGN_UP_SUCCESS = "SIGN_UP_SUCCESS"
export const SIGN_UP_ERROR = "SIGN_UP_ERROR"

export const LOG_IN_REQUEST = "LOG_IN_REQUEST"
export const LOG_IN_SUCCESS = "LOG_IN_SUCCESS"
export const LOG_IN_ERROR = "LOG_IN_ERROR"

export const VIEW_POLL_REQUEST = "VIEW_POLL_REQUEST"
export const VIEW_POLL_SUCCESS = "VIEW_POLL_SUCCESS"
export const VIEW_POLL_ERROR = "VIEW_POLL_ERROR"

export const ADD_NEW_POLL_REQUEST = "ADD_NEW_POLL_REQUEST"
export const ADD_NEW_POLL_SUCCESS = "ADD_NEW_POLL_SUCCESS"
export const ADD_NEW_POLL_ERROR = "ADD_NEW_POLL_ERROR"

export const DELETE_POLL_REQUEST = "DELETE_POLL_REQUEST";
export const DELETE_POLL_SUCCESS = "DELETE_POLL_SUCCESS";
export const DELETE_POLL_ERROR = "DELETE_POLL_ERROR";

export const LIST_USERS_REQUEST = "LIST_USERS_REQUEST";
export const LIST_USERS_SUCCESS = "LIST_USERS_SUCCESS";
export const LIST_USERS_ERROR = "LIST_USERS_ERROR";

export const VOTE_REQUEST = "VOTE_REQUEST";
export const VOTE_SUCCESS = "VOTE_SUCCESS";
export const VOTE_ERROR = "VOTE_ERROR";

export const EDIT_POLL_REQUEST = "EDIT_POLL_REQUEST";
export const EDIT_POLL_SUCCESS = "EDIT_POLL_SUCCESS";
export const EDIT_POLL_ERROR = "EDIT_POLL_ERROR";

export const DELETE_POLL_OPTION_REQUEST = "DELETE_POLL_OPTION_REQUEST";
export const DELETE_POLL_OPTION_SUCCESS = "DELETE_POLL_OPTION_SUCCESS";
export const DELETE_POLL_OPTION_ERROR = "DELETE_POLL_OPTION_ERROR";

export const LIST_A_POLL_REQUEST = "LIST_A_POLL_REQUEST";
export const LIST_A_POLL_SUCCESS = "LIST_A_POLL_SUCCESS";
export const LIST_A_POLL_ERROR = "LIST_A_POLL_ERROR";

export const ADD_NEW_POLL_OPTION_REQUEST = "ADD_NEW_POLL_OPTION_REQUEST"
export const ADD_NEW_POLL_OPTION_SUCCESS = "ADD_NEW_POLL_OPTION_SUCCESS"
export const ADD_NEW_POLL_OPTION_ERROR = "ADD_NEW_POLL_OPTION_ERROR"